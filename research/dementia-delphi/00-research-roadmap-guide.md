...
- 연구대상을 **4집단(정상 / MCI / 치매[CDR 0.5] / 치매[CDR 1])**으로 분류 확정한다. **CDR**을
  1차 기준으로 하되, MCI와 치매(CDR 0.5)는 점수가 겹치므로 **진단기준(기능손상 유무)을 병용**해
  나눈다 (Morris, 1993; Reisberg et al., 1982). 중등도·중증(CDR 2~3)은 범위에서 제외.
  → 상세 표·조작적 정의는 `01-research-design.md` 4절, **판정 흐름도는
  `05-classification-algorithm.md`**.
...
2. **치매 조기검진 평가자료** — **"인지단계 분류 축"** 역할.
   - 신경심리 배터리(MMSE·단어목록기억·보스턴이름대기·단어유창성 = CERAD-K), SNSB, CDR·GDS → 대상자가 **어느 단계**인지 규정.
...
> | 성격 | 횡단·지역사회·**초기(정상~MCI)** | 종단·임상·**초기 치매(CDR 0.5~1) 간호 내용** |
...
| 인지단계(4집단) | 간호진단(NANDA-I) | 간호중재(NIC) | 간호평가(NOC) |
|----------------|-------------------|---------------|---------------|
| ① 정상(검진 정상군) | … | … | … |
| ② MCI (CDR 0.5·진단기준 미충족) | … | … | … |
| ③ 치매 CDR 0.5 (최경도) | … | … | … |
| ④ 치매 CDR 1 (경도) | … | … | … |
...
- [ ] **4집단 분류 규칙표**: CDR 0=정상 / CDR 0.5 → 진단기준으로 MCI vs 치매(CDR 0.5) 구분 /
      CDR 1=치매(경도). (MCI와 치매 0.5의 조작적 구분 기준을 사전 명문화)
...
- ✗ **MCI와 치매(CDR 0.5)를 CDR 점수만으로 가르기** → 둘 다 0.5라 구분 불가. 반드시
  진단기준(기능손상 유무)을 병용하고, 그 조작적 정의를 미리 못박아야 한다.
- ✗ (참고) 중등도·중증은 **의도적으로 범위 제외**했으므로, 결과를 초기 단계(정상~CDR 1)에
  한정해 해석한다. 범위를 넘는 일반화는 하지 않는다.
...
- `05-classification-algorithm.md` — 4집단 분류 판정 흐름도(CDR 라우팅 + ②/③ 신경심리[MMSE·단어목록기억·보스턴이름대기·단어유창성]·기능평가 세부 알고리즘)

## 전체 진행 체크리스트 (한 장 요약)
- [ ] 0. 주제·목적 확정(합의·내용타당화로 프레이밍) / **4집단 분류(정상·MCI·치매 0.5·치매 1,
      CDR+진단기준)** / IRB 승인
...
> 국내 한국어 문헌(치매안심센터·신경심리 도구 등)은 PubMed 범위 밖이므로 RISS/KCI/DBpia로 보강한다(`02` §2-9).
